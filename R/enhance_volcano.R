...
#'
#' @examples
#' enhance_volcano(deg_data,x = 'log2FoldChange', y = 'pvalue',
...
enhance_volcano <- function(deg_data,x, y,
  ...
  if (cut_FC == "auto") {
    cut_FC = mean(abs(deg_data[,x])) + 2*sd(abs(deg_data[,y]))
  }
  ...
  plot <- enhance_volcano_plot(deg_data_g,x = 'log2FoldChange', y = 'pvalue', label = label, cut_FC = cut_FC, cut_P = cut_P)
...
enhance_volcano_plot <- function(data,x,y,label = c("Down","Stable","Up"),cut_FC,cut_P, palette = c('#66c2a5', 'grey50', '#fc8d62')) {
...
#'
#' @examples
#' top_deg(deg_data, x = "log2FoldChange", y = "pvalue", 50, 0.05)
...
#'
#' @examples
#' cut_much(deg_data, x = "log2FoldChange", y = "pvalue", cut_FC = 1,cut_P = 0.05,label = c("Down","Stable","Up"), label_ns = "Stable")
...
#'
#' @param deg_data a deg data frame
#' @param genes_list a gene names character vectors;
#' @param highlight a gene names character vectors; default NULL
...
#' @export
#' @noRd
show_genes <- function(deg_data, x = "log2FoldChange", y = "pvalue", genes_list, highlight = NULL,p,size = 1.8, expand = c(0.12, 0.12)) {

  max_lfc <- max(deg_data[,x])
  min_lfc <- min(deg_data[,x])

  max_fdr <- max(-log10(deg_data[,y]))

  if (max_fdr) {
    max_fdr <- 300
  }

  deg_data <- data.table(deg_data, key=x, keep.rownames = TRUE)
  ...
  if (any(!is.null(genes_list),!is.null(highlight))) {
    ...
    if (!is.null(genes_list)) {
      ...
      label_data <- merge(genes_list,deg_data)
    }
    ...
    if (!is.null(highlight)) {
      ...
      hlight_data <- merge(highlight,deg_data)
    }
